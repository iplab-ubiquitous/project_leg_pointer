...
class draw_gui(QWidget):
    ...
    def set_target(self):
        for i in range(num_of_targets):
            cx = window_size_x / 2 + radius*math.cos(2 * math.pi * (float(i) / num_of_targets))
            cy = window_size_y / 2 + radius*math.sin(2 * math.pi * (float(i) / num_of_targets))
    ...
    def paintEvent(self, QPaintEvent):
        ...
        for i in range(num_of_targets):
            painter.drawEllipse(self.target_point[i], target_size, target_size)
        painter.setBrush(QColor(0x00, 0x00, 0xff))
        painter.drawEllipse(self.target_point[self.target_num], target_size, target_size)
        if self.collision_flag:
            painter.setBrush(Qt.white)
            painter.drawEllipse(self.target_point[self.collision_num], 40, 40)
        ...
        if self.calibration_check():
            ...
            for i in range(num_of_targets):
                self.collision_flag = ((self.x - self.target_point[i].x()) * (self.x - self.target_point[i].x())) + (
                    (self.y - self.target_point[i].y()) * (self.y - self.target_point[i].y())) <= (target_size + pointer_size) * (target_size + pointer_size)
                if self.collision_flag:
