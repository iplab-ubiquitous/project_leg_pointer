...
plt.title("右膝の選択時間（水平操作のみ抽出）", fontsize=16)
...
throughput[0], error[0] = fitts_fit(
    "P1", "./leg/data_p1_right_horizontal.csv", 0, '^')
throughput[1], error[1] = fitts_fit(
    "P2", "./leg/data_p2_right_horizontal.csv", 1, 'x')
throughput[2], error[2] = fitts_fit(
    "P3", "./leg/data_p3_right_horizontal.csv", 2, '+')
throughput[3], error[3] = fitts_fit(
    "P4", "./leg/data_p4_right_horizontal.csv", 3, '+')
...
plt.ylim(0,10)
plt.xlim(0.0,5.5)
plt.savefig('Horizontal_right.png')
