...
num_of_targets = 4 #分析対象とする選択回数の指定
...
plt.title("右膝の選択時間（垂直操作のみ抽出）", fontsize=16)
...
throughput[0], error[0] = fitts_fit(
    "P1", "./leg/data_p1_right_vertical.csv", 0, '^')
throughput[1], error[1] = fitts_fit(
    "P2", "./leg/data_p2_right_vertical.csv", 1, 'x')
throughput[2], error[2] = fitts_fit(
    "P3", "./leg/data_p3_right_vertical.csv", 2, '+')
throughput[3], error[3] = fitts_fit(
    "P4", "./leg/data_p4_right_vertical.csv", 3, '+')
...
plt.ylim(0,10)
plt.xlim(0.0,5.5)
plt.savefig('Vertical_right.png')
