...
def fitts_fit(name, path, cmap_num, shape):
        ...
        plt.scatter(id_effective_forfit, time_raw, s=5, marker=shape, c=cmap(cmap_num), label="選択時間({})".format(name))
        p = np.poly1d(fitted)(xp)
        plt.plot(xp, p, label=("選択予想時間 ({}): {}+{}*ID, r2:{}".format(name, round(fitted[1], 3), round(fitted[0], 3), r2)))

        return statistics.mean(tp), err
throughput[0], error[0] = fitts_fit("P1", "./leg/data_p1_right_leg.csv", 0, '^')
throughput[1], error[1] = fitts_fit("P2", "./leg/data_p2_right_leg.csv", 1, 'x')
throughput[2], error[2] = fitts_fit("P3", "./leg/data_p3_right_leg.csv", 2, '+')
throughput[3], error[3] = fitts_fit("P4", "./leg/data_p4_right_leg.csv", 3, '+')
